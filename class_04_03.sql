...
-- UPDATA 구문을 사용할 때는 천천히 WHERE 확인하기
-- 1. id가 1인 주문으 ㅣ상품 수량을 5개로 수정
...
-- 3. id가5인 주문의 주문 일자를 `2024-05-31` 로 수정
update `order`
...
-- 주문날짜가 '2022-02-28'이고 ,상품ㅁ명이 '초밥'인 주문의 주문수량 5개로 수정
update `order`
set quantity=5
...
-- SQL
-- DDL(Data Definition Language) : 테이블 구조를 변경할 때 사용됩니다
-- DML
-- DCL
...
-- 주문 수량이 1개 이하이고, 상품명이 '감바스'인 주문의 수령주소를 '경기도 성남시'로 수정
update `order`
set address='부산시 진구'
