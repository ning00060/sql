...
/*
 customer 테이블과 입력되어 있는 데이터를 활용해서 문제를 직접 만들어보고 쿼리 구문을 작성 해주세요 문제는 최소 5개 이상입니다.
...
-- null 찾을 때 사용 방법 --> 공백도 데이터이고 빈 문자열도 데이터 이다.
select *
from customer
where email is null;
...
-- 3. id가 5이상이며 주소가 서울시에 거주하는 사람 이름을 검색해라 
  ...
  select *
from customer
where id>=5 and address in('서울시 강남구','서울시 서초구','서울시 송파구','서울시 강서구');  
